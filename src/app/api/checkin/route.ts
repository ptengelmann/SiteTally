...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<Asset>>> {
  ...
  try {
    ...
    // 2. Check if asset is currently checked out
    if (asset.current_status !== 'CHECKED_OUT') {
      await client.query('ROLLBACK');
      return NextResponse.json(
        { success: false, error: `Asset is not checked out. Current status: ${asset.current_status}` },
        { status: 409 }
      );
    }
    ...
    await client.query(
      ...
      [asset.asset_id, user_id, checkinTime, asset.current_location, notes || null]
    );
    ...
    return NextResponse.json(
      { success: false, error: 'Internal server error during checkin' },
      { status: 500 }
    );
  } finally {
    client.release();
